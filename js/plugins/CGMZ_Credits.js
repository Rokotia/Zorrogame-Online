 ...
 * rename the js file.
 * -----------------------------Latest Version---------------------------------
 * Hi all, this latest version converts the windowskin, etc. parameters to use
 * a [CGMZ] Window Settings integration instead. Now you provide the preset id
 * from that plugin to edit the window settings in this plugin. This allows me
 * to add more window options without needing to update every plugin with
 * windows separately, and makes the plugins overall easier to use.
 *
 * Version 1.2.1
 ...
 * @parent Window Options
 * @type number
 * @desc Amount of blank space (in pixels) to leave between credit categories
 * @default 12
 *
 * @param Window Width
 * @parent Window Options
...
CGMZ.Credits.CategorySpace = Number(CGMZ.Credits.parameters["Category Space"]);
...
CGMZ.Credits.WindowSettings = CGMZ.Credits.parameters["Window Settings"];
...
CGMZ_Window_Credits.prototype.initialize = function(rect) {
	...
	if(Imported.CGMZ_WindowSettings && CGMZ.Credits.WindowSettings) this.CGMZ_setWindowSettings(CGMZ.Credits.WindowSettings);
...
CGMZ_Window_Credits.prototype.refresh = function() {
	...
	if(CGMZ.Credits.WindowTitle) this.drawTitle();
	this.drawCredits();
};
//-----------------------------------------------------------------------------
...
CGMZ_Window_Credits.prototype.drawCredits = function() {
	for(const credit of $cgmzTemp.getCredits()) {
		this.contents.fontBold = true;
		this._neededHeight += this.CGMZ_drawText(credit.categoryName, 0, 0, this._neededHeight, this.contents.width, 'center');
		this.contents.fontBold = false;
		const creditEntries = credit.credits;
		for(const entry of creditEntries) {
			this._neededHeight += this.CGMZ_drawText(entry.Name, 0, 0, this._neededHeight, this.contents.width, 'center');
			if(entry.Comment) {
				this._neededHeight += this.CGMZ_drawText(entry.Comment, 0, 0, this._neededHeight, this.contents.width, 'center');
			}
			if(entry.URL) {
				const urlHeight = this.CGMZ_drawText(entry.URLText, 0, 0, this._neededHeight, this.contents.width, 'center');
				let urlObject = {};
				urlObject.url = entry.URL;
			...
			this._neededHeight += CGMZ.Credits.CategorySpace;
		}
	}
	this._neededHeight -= CGMZ.Credits.CategorySpace; // Remove custom space after last entry
};
//-----------------------------------------------------------------------------
// Update
//-----------------------------------------------------------------------------
